...
**Your Answers here:**
<br>
1. "dataSet1.R" launched download of ticker AAPL
<br>
2. 
...
<br> 1. nrow(StockReturns); ncol(StockReturns)
<br>
<br>2. Use apply(transposedData,2,mean) to calculate the mean for the columns of the transposedData matrix. It is an average of the return for AAPL and SPY, 50/50 split.
<br>
...
**Your Answers here:**
<br>
<br>1. Change to startDate = "2001-01-01" (already in the file)
<br>
<br>2. Yes, first add companies to mytickers, e.g. ("TIF", "COH"), then change the start date. Then we can run it again
<br>
<br>
...
**Your Answers here:**
<br>
<br>1. This command shows the number of times myData is not equal to StockReturns[1:20] = `r sum(myData != StockReturns[1:20,])`
<br>2. The tables will be combined together, resulting in doubles for the first 20 rows. After the first 20 rows, myData will be added again to the second 20 numbers of StockReturns.
<br>
<br>
